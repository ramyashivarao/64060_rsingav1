---
title: "Assignment_2_MIS_64060-001.Rmd"
output:
...
Goal : Use k Nearest Neighbors model to predict whether a new customer will accept a loan offer.
...
# Conclusions: 
We can see that the accuracy is highest for the training set, followed by the validation set, and then the test set. This is expected, as the model is trained on the training set, so it performs better on that set compared to the validation and test sets. However, if the model overfits to the training set, it may not generalize well to new data, leading to lower accuracy on the validation and test sets. In this case, we can see that the accuracy on the validation and test sets is still quite high, which suggests that the model is not overfitting and is likely to generalize well to new data.

Also, We may expect to see some differences between the confusion matrices of the training, validation, and test sets because each set represents a different sample of the population, and there may be variations in the distribution of the predictor variables across the sets.

Accuracy/Precision seems to be close to each other in all the datasets which may indicate that the model is well generalized to new data and is not prone to overfitting.

However, Recall seems to be the metric which has a huge discrepancy across the above 3 data sets which could be due to many reasons: Overfitting, Imbalanced Classes and Data Quality. 

Imbalanced classes might not be the issue here as we do stratified sampling.
